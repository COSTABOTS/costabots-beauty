...
function ServiceEditor({ businessCurrency, onCancel, onSave, service }: { businessCurrency: string; onCancel: () => void; onSave: (value: ServiceInput) => Promise<void>; service?: BeautyService }) {
  const [value, setValue] = useState<ServiceInput>({ name: service?.name ?? '', description: service?.description ?? '', durationMinutes: service?.durationMinutes ?? 60, bufferBeforeMinutes: service?.bufferBeforeMinutes ?? 0, bufferAfterMinutes: service?.bufferAfterMinutes ?? 0, price: service?.price ?? 0, currency: service?.currency ?? businessCurrency, onlineBookingEnabled: service?.onlineBookingEnabled ?? true, reactivationDays: service?.reactivationDays ?? null });
  ...
  return <form className="beauty-command-form" onSubmit={(e) => { e.preventDefault(); void onSave(value); }}><h2>{service ? 'Editar servicio' : 'Nuevo servicio'}</h2>
    <label><span>Nombre *</span><input required value={value.name} onChange={(e) => set('name', e.target.value)} /></label>
    ...
    <details className="advanced-options"><summary>Opciones avanzadas</summary><label><span>Descripción</span><textarea value={value.description} onChange={(e) => set('description', e.target.value)} /></label>
      <div className="form-columns"><label><span>Tiempo antes</span><input min={0} type="number" value={value.bufferBeforeMinutes} onChange={(e) => set('bufferBeforeMinutes', Number(e.target.value))} /></label><label><span>Tiempo después</span><input min={0} type="number" value={value.bufferAfterMinutes} onChange={(e) => set('bufferAfterMinutes', Number(e.target.value))} /></label></div>
      <div className="form-columns"><label><span>Moneda</span><input maxLength={3} value={value.currency} onChange={(e) => set('currency', e.target.value.toUpperCase())} /></label><label><span>Recordar al cliente (días)</span><input min={1} type="number" value={value.reactivationDays ?? ''} onChange={(e) => set('reactivationDays', e.target.value ? Number(e.target.value) : null)} /></label></div>
      <label className="check-row"><input checked={value.onlineBookingEnabled} type="checkbox" onChange={(e) => set('onlineBookingEnabled', e.target.checked)} />Disponible para reserva online</label></details>
    <div className="form-actions"><button type="button" onClick={onCancel}>Cancelar</button><button type="submit">Guardar</button></div>
  </form>;
}
