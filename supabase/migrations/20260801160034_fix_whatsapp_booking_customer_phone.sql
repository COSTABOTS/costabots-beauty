-- Evolution stores the remote WhatsApp identifier as international digits.
-- Customers require canonical E.164, so normalize it inside the trusted RPC.
...
begin
  ...
  v_customer_phone := '+' || regexp_replace(v_conversation.remote_phone_normalized, '[^0-9]', '', 'g');
  if v_customer_phone !~ '^\+[1-9][0-9]{7,14}$' then
    raise exception 'BOOKING_CUSTOMER_PHONE_INVALID' using errcode = '22023';
  end if;
