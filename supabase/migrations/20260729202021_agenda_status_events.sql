-- Preserve the simplified status flow while recording a precise audit event.
...
declare
  ...
  v_event_type public.beauty_appointment_event_type;
begin
  ...
  for update;
  if not found then
    raise exception 'Appointment not found in this Beauty business' using errcode = 'P0002';
  end if;
  if not (
    public.has_business_role(p_business_id, array['owner', 'admin'])
    or public.current_staff_member_id(p_business_id) = v_appointment.assigned_staff_member_id
  ) then
  ...
  if not (
    ...
    or (v_previous_status = 'confirmed' and p_new_status in ('completed', 'no_show'))
  ...
  v_event_type := case p_new_status
    when 'confirmed' then 'confirmed'::public.beauty_appointment_event_type
    when 'completed' then 'completed'::public.beauty_appointment_event_type
    when 'no_show' then 'no_show'::public.beauty_appointment_event_type
  end;
  ...
  insert into public.appointment_events (
    ...
    p_business_id, p_appointment_id, v_event_type,
